...
A **production-style Spring Boot REST API** for managing users with full CRUD, pagination, sorting, validation, DTOs, and global exception handling.
...
The **Spring Boot User Management API** exposes a clean REST API to create, read, update, and delete users stored in MySQL. The codebase follows **layered architecture** (Controller → Service → Repository), separates **entities from DTOs**, and returns consistent JSON envelopes via `ResponseEntity` and a global exception handler.
...
├── postman/
│   └── SpringBoot-User-Management-API.postman_collection.json
...
5. **Import Postman collection:**  
   `postman/SpringBoot-User-Management-API.postman_collection.json`  
